...
from plot_ROC import main as ROC_main
from plot_ROC import plot_ROC as ROC_plot
...
def plot_ROC(Y_test, Y_score, N1, N2, path, label, colour, alpha=0.95, tsamples=40, plot_conf=True):
    ...
    plt.plot(CIs_fpr_means, CIs_tpr_means, color=colour, lw=lw, label=label)
...
def main(arg_list):
    ...
    for folder in arg_list:
        ...
        #Execute
        colours = plt.cm.rainbow(np.linspace(0, 1, len(arg_list)-1))
        if 'SVMp' in folder:
            label = 'Polynomial'
            colour = 'red'
        elif 'SVMl' in folder:
            ...
            label = 'RBF'
            colour = 'green'
        plot_ROC(Y_test, Y_score, N1, N2, target_folder, label=label, colour=colour, tsamples=n_samples, plot_conf=False)
    ...
    image_name = "ROCs_combined.png"

    for folder in arg_list:
        target_folder = '/archive/wkessels/output/{}/'.format(folder)
        plt.savefig(target_folder + image_name)
        print 'ROC curve saved to ' + target_folder
